...
> Start Claude Code, Gemini CLI, and Aider side by side. Each session runs in tmux, so the process can survive backend restarts.
...
> One agent can implement, another can review, and the team can follow the whole workflow from one place.
...
>
> Multi-agent terminals, tmux-backed sessions, chat routing, and task tracking in one workspace.
...
> I wanted a way to coordinate Claude Code, Gemini CLI, and Aider without juggling separate terminals, so I built Orchestra: a browser cockpit with tmux-backed agent sessions, chat routing, and task tracking.
