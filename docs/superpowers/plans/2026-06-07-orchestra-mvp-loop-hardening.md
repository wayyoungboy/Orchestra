...
- Added a workspace Agent Sessions navigation tab that lists active backend sessions, owning members, current tmux pane snapshots, stream output, controlled single-message terminal input, and stale-session termination.

The next product gap is a full xterm-style terminal surface. The member-card session action and Agent Sessions tab make agent startup, ownership, current screen state, output, light input, and session cleanup usable before a complete terminal emulator UI is rebuilt.
