...
- Aligned Kanban drag-and-drop task transitions with the backend task state machine, preventing browser-only moves such as pending directly to completed/failed that would be rejected by the API.
...
- Added a manual GitHub Actions `workflow_dispatch` switch for the full focused browser MVP E2E gate, using the runner's system Chrome/Chromium executable, so maintainers can run `make verify-focused` remotely with a temporary backend before broader workflow QA.
