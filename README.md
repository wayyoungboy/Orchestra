...
The Members page now exposes that source of truth directly: configured assistants and secretaries can start or reuse their backend agent session from the member card, and failed starts surface inline instead of disappearing behind chat dispatch.
...
- **Member-Level Agent Launch**: Start or reuse an assistant/secretary backend session from the member card using the saved CLI/ACP command
