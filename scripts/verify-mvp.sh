...
if [[ "$run_all_focused_e2e" == "1" || "${ORCHESTRA_RUN_MVP_DM_E2E:-}" == "1" ]]; then
  echo "==> MVP direct message flow E2E"
...
if [[ "$run_all_focused_e2e" == "1" || "${ORCHESTRA_RUN_MVP_NOTIFICATION_E2E:-}" == "1" ]]; then
  echo "==> MVP notification flow E2E"
  ORCHESTRA_SKIP_FRONTEND_BUILD=1 "$ROOT_DIR/scripts/run-mvp-notification-e2e.sh"
fi

if [[ "$run_all_focused_e2e" == "1" || "${ORCHESTRA_RUN_MVP_DISPATCH_DIAGNOSTICS_E2E:-}" == "1" ]]; then
  echo "==> MVP dispatch diagnostics flow E2E"
  ORCHESTRA_SKIP_FRONTEND_BUILD=1 "$ROOT_DIR/scripts/run-mvp-dispatch-diagnostics-e2e.sh"
fi
