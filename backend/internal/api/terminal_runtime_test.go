...
func TestRapidMentionedMessagesReachSameAssistantTerminal(t *testing.T) {
	if !tmuxRuntimeAvailable() {
		t.Skip("tmux not installed")
	}
	...
	workspaceResp := requestJSON(t, router, http.MethodPost, "/api/workspaces", map[string]any{
		"name":             "Rapid Dispatch Workspace",
		"path":             workspaceDir,
		"ownerDisplayName": "Rapid Owner",
	})
	if workspaceResp.Code != http.StatusCreated {
		t.Fatalf("create workspace status = %d body=%s", workspaceResp.Code, workspaceResp.Body.String())
	}
	...
	markers := []string{"ORCH_RAPID_ONE", "ORCH_RAPID_TWO"}
	for _, marker := range markers {
		messageResp := requestJSON(t, router, http.MethodPost, "/api/workspaces/"+workspaceID+"/conversations/"+conversationID+"/messages", map[string]any{
			"text":       "Please queue " + marker,
			"senderId":   "owner",
			"senderName": "Rapid Owner",
			"mentionIds": []string{memberID},
		})
		if messageResp.Code != http.StatusCreated {
			t.Fatalf("send message %s status = %d body=%s", marker, messageResp.Code, messageResp.Body.String())
		}
	}
	...
	deadline = time.Now().Add(8 * time.Second)
	for time.Now().Before(deadline) {
		snapshotResp := requestJSON(t, router, http.MethodGet, "/api/terminals/"+sessionID+"/snapshot?lines=240", nil)
		if snapshotResp.Code != http.StatusOK {
			t.Fatalf("snapshot status = %d body=%s", snapshotResp.Code, snapshotResp.Body.String())
		}
		snapshot = decodeJSON[map[string]any](t, snapshotResp)
		content, _ := snapshot["content"].(string)
		if strings.Contains(content, "#conversationId{"+conversationID+"}") &&
			strings.Contains(content, markers[0]) &&
			strings.Contains(content, markers[1]) {
			return
		}
		time.Sleep(100 * time.Millisecond)
	}
	content, _ := snapshot["content"].(string)
	t.Fatalf("rapid dispatch prompts did not both reach assistant terminal; wanted %v in %#v", markers, content)
}
