...
Orchestra 默认只监听 `127.0.0.1:8080`。在实现“登录用户—工作区”授权关系前，它按单管理员控制台设计；未启用认证时请不要暴露到公网或局域网。

远程部署应在首次启动前配置以下密钥：

```bash
export ORCHESTRA_JWT_SECRET='至少 32 字节的随机字符串'
export ORCHESTRA_ADMIN_USERNAME='admin'
export ORCHESTRA_ADMIN_PASSWORD='足够长且唯一的密码'
export ORCHESTRA_ENCRYPTION_KEY='至少 32 字节的随机字符串'
```

保存供应商 API Key 需要 `ORCHESTRA_ENCRYPTION_KEY`；程序不会再回退到可预测的开发密钥。工作区级用户授权完成前，自助注册保持关闭。
...
| `/api/workspaces/:id/conversations` | GET/POST | 获取或创建会话 |
| `/api/workspaces/:id/conversations/:convId/messages` | GET/POST | 读取或发送消息 |
| `/api/workspaces/:id/tasks` | GET | 列出工作区任务 |
| `/api/internal/tasks/*` | POST | 智能体任务生命周期回调 |
| `/api/api-keys` | GET/POST | API 密钥管理 |
| `/api/api-keys/:id` | DELETE | 删除 API 密钥 |
| `/api/api-keys/test` | POST | 测试 API 密钥 |
...
- `ORCHESTRA_JWT_SECRET` - 启用 JWT 认证
- `ORCHESTRA_ADMIN_USERNAME` / `ORCHESTRA_ADMIN_PASSWORD` - 首次启用认证时创建管理员所必需
...
像 `codex` 这样的命令名只允许通过 PATH 查找；绝对可执行路径必须显式写入白名单。除非明确需要且完全信任，否则不要把 shell 加入白名单。
