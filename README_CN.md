...
成员页已经直接暴露这个入口：配置过命令的 assistant / secretary 可以从成员卡片启动或复用后端 agent 会话，启动失败会显示在卡片内，不再只依赖聊天派发时才暴露问题。
...
- **成员级 Agent 启动**：从成员卡片使用已保存的 CLI/ACP 命令启动或复用 assistant / secretary 后端会话
